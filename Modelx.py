...
class Embedding(nn.Module):
    def __init__(self, in_channels=3, patch_height=64, emb_size=768):
        super(Embedding, self).__init__()
        self.patch_height = patch_height
        self.projection = nn.Sequential(
            Rearrange('b c (ph h) (w) -> b (h w) (ph c)', ph=self.patch_height),
            nn.Linear(patch_height * in_channels, emb_size)
        )
...
class ViT(nn.Module):
    ...
    def forward(self, x):
        ...
        b, n, _ = x.shape
        print(f'Embedding shape: {b}x{n}x{_}')
